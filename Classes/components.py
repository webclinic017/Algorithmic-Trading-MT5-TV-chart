...
def side_bar(frame):
    ...
    frame.logo_label = customtkinter.CTkLabel(frame.sidebar_frame, text="Menu", font=customtkinter.CTkFont(size=20, weight="bold"))
    frame.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))
    frame.sidebar_button_1 = customtkinter.CTkButton(frame.sidebar_frame, command=frame.sidebar_button_login_event,text="Log In")
    frame.sidebar_button_1.grid(row=1, column=0, padx=20, pady=10)
    
    frame.sidebar_button_2 = customtkinter.CTkButton(frame.sidebar_frame, command=frame.sidebar_button_event,text="Help")
    frame.sidebar_button_2.grid(row=2, column=0, padx=20, pady=10)      
    frame.appearance_mode_label = customtkinter.CTkLabel(frame.sidebar_frame, text="Appearance Mode:", anchor="w")
    frame.appearance_mode_label.grid(row=5, column=0, padx=20, pady=(10, 0))
    frame.appearance_mode_optionemenu = customtkinter.CTkOptionMenu(frame.sidebar_frame, values=["Custom", "Dark","Light"],
                                                                    command=frame.change_appearance_mode_event)
    frame.appearance_mode_optionemenu.grid(row=6, column=0, padx=20, pady=(10, 10))
    frame.scaling_label = customtkinter.CTkLabel(frame.sidebar_frame, text="UI Scaling:", anchor="w")
    frame.scaling_label.grid(row=7, column=0, padx=20, pady=(10, 0))
    frame.scaling_optionemenu = customtkinter.CTkOptionMenu(frame.sidebar_frame, values=["80%", "90%", "100%", "110%", "120%"],
                                                            command=frame.change_scaling_event)
    frame.scaling_optionemenu.grid(row=8, column=0, padx=20, pady=(10, 20))
...
def strategy_running_screen(frame):
    ...
    #frame.monitor_session_thread.start()
    # Update the PL
    update_profit(frame)    
...
def start_strategy_mt5_screen(frame):
    ...
    """
    def display_elements(selection):        
        if selection == "Enable":
            i = 0
            for element in hidden_elements:
                if i == 0:
                    element.grid(row=13, column=i, padx=PADX, pady=PADY)
                else:
                    element.grid(row=13, column=i, padx=PADX, pady=PADY,sticky="ew")
                i += 1
            ...
            frame.main_frame.partial_close_directions_menu.set("Enable")
        else:
            for element in hidden_elements:
                element.grid_forget()
            frame.main_frame.message.grid(row=1, column=0, columnspan=2, padx=(20, 20), pady=(20, 10), sticky="ew")
            frame.main_frame.start_strategy.grid(row=13, column=0, columnspan=2, padx=150, pady=15, sticky="ew")
            frame.main_frame.partial_close_directions_menu.set("Disable")
    ...
    points = lambda x: 800 if x == "XAUUSD" else 65
    ...
    if len(symbols) != 0:       
        ...
        frame.main_frame.points_label.grid(row=10, column=0, padx=PADX, pady=PADY)
        ...
        frame.main_frame.points_entry.grid(row=10, column=1, padx=PADX, pady=PADY, sticky="ew")        
        ...
        frame.main_frame.dynamic_SL_label.grid(row=11, column=0, padx=PADX, pady=PADY)
        frame.main_frame.dynamic_SL_menu = customtkinter.CTkOptionMenu(frame.main_frame,   
                                                                       values=["Enable", "Disable"])
        frame.main_frame.dynamic_SL_menu.grid(row=11, column=1, padx=PADX, pady=PADY,sticky="ew")
        # Reverse entries
        frame.main_frame.reverse_label = customtkinter.CTkLabel(frame.main_frame,
                                                                        text="Reverse entries:", anchor="e")
        frame.main_frame.reverse_label.grid(row=12, column=0, padx=PADX, pady=PADY)
        frame.main_frame.reverse_menu = customtkinter.CTkOptionMenu(frame.main_frame,   
                                                                       values=["Enable", "Disable"])
        frame.main_frame.reverse_menu.grid(row=12, column=1, padx=PADX,sticky="ew")
        
        # Partial Close both directions    
        frame.main_frame.partial_close_label = customtkinter.CTkLabel(frame.main_frame,
                                                                        text="Partial Close in \nboth directions (P/L):", anchor="e")
        frame.main_frame.partial_close_label.grid(row=13, column=0, padx=PADX, pady=PADY)
        frame.main_frame.partial_close_directions_menu = customtkinter.CTkOptionMenu(frame.main_frame,
                                                                                            values=["Enable", "Disable"])        
        frame.main_frame.partial_close_directions_menu.grid(row=13, column=1, padx=PADX, pady=PADY, sticky="ew")
        hidden_elements = [frame.main_frame.partial_close_label,frame.main_frame.partial_close_directions_menu]
