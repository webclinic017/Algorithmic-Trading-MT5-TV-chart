...
def start_strategy_mt5_screen_optmized(frame):
    """
    Display the screen with the current symbols and parameters to start the strategy
    using a two-column layout.
    """
    ...
    if len(symbols) != 0:       
        ...
        frame.main_frame.symbols_options = customtkinter.CTkLabel(frame.main_frame,text=f"{frame.symbol}", anchor="e")
        frame.main_frame.symbols_options.grid(row=3, column=1, padx=PADX, pady=(10, 10), sticky="ew")
        # Risk 
        frame.main_frame.risk = customtkinter.CTkLabel(frame.main_frame, text="% Risk:",
                                                                        anchor="e")
        ...
        frame.main_frame.points_label = customtkinter.CTkLabel(frame.main_frame,
                                                            text="Points to TP/SL:", anchor="e")
        frame.main_frame.points_label.grid(row=10, column=0, padx=PADX, pady=PADY)
        frame.main_frame.points_entry = customtkinter.CTkLabel(frame.main_frame,                                                           
                                                            text=f"{frame.points}", anchor="e")        
        frame.main_frame.points_entry.grid(row=10, column=1, padx=PADX, pady=PADY, sticky="ew")        
        # Dynamic SL
        frame.main_frame.dynamic_SL_label = customtkinter.CTkLabel(frame.main_frame,
                                                                        text="Dynamic SL:", anchor="e")
        ...
        frame.main_frame.reverse_label = customtkinter.CTkLabel(frame.main_frame,
                                                                        text="Reverse entries:", anchor="e")
        frame.main_frame.reverse_label.grid(row=12, column=0, padx=PADX, pady=PADY)
        frame.main_frame.reverse_menu = customtkinter.CTkLabel(frame.main_frame,   
                                                                       text=f"{frame.reverse}", anchor="e")
        frame.main_frame.reverse_menu.grid(row=12, column=1, padx=PADX,sticky="ew")
...
def fine_tunning_strategy(frame):
    """
    This screen is used to finetune the strategy before running the strategy
        -This will run the fucntions to set te recommendend paramateres
    """    
    ...
    frame.main_frame.grid_rowconfigure(2, weight=0)
    frame.main_frame.grid_rowconfigure(3, weight=0)
    frame.main_frame.grid_rowconfigure(4, weight=1)
    ...
    if len(symbols) != 0:                
        ...
        frame.main_frame.message = customtkinter.CTkLabel(frame.main_frame,
                                                                            text="Select Symbol",
                                                                            font=customtkinter.CTkFont(size=22, weight="bold"))
        frame.main_frame.message.grid(row=0, column=0, columnspan=2, padx=20, pady=50, sticky="ew")  
      
        # Symbol Selector
        frame.main_frame.symbol = customtkinter.CTkLabel(frame.main_frame, text="Symbol:", 
                                                          font=customtkinter.CTkFont(size=16),
                                                          anchor="e")
        frame.main_frame.symbol.grid(row=1, column=0, padx=0, pady=PADY)

        frame.main_frame.symbols_options = customtkinter.CTkOptionMenu(frame.main_frame,
                                                                        values=[symbol.name for symbol in symbols])  
        frame.main_frame.symbols_options.grid(row=1, column=1, padx=PADX, pady=PADY, sticky="ew")  
                                              
        # START button 
        frame.main_frame.start_strategy = customtkinter.CTkButton(frame.main_frame, text="START", command=frame.start_optimization)
        frame.main_frame.start_strategy.grid(row=3, column=0, columnspan=2, padx=150, pady=15, sticky="ew")
